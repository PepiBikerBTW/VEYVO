...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    const result=await win.webContents.executeJavaScript(`(async()=>{
      ...
      state.language='cs';state.weekPlans['1']=[['PO','Regenerace','Volno','—'],['ÚT','Lehký běh','5 km','5 km'],...baseWorkouts.slice(2)]; refreshCalendar();
      ...
      expect($('#messages').lastElementChild.textContent.includes('připoj OpenAI'),'No simulated AI without connection');
      sendCoachMessage('Spal jsem jen 5 hodin');await new Promise(r=>setTimeout(r,500));
      expect($('#messages').lastElementChild.textContent.includes('připoj OpenAI'),'No canned sleep reply');
